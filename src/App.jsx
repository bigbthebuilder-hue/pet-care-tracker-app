...
const VERSION = "PET_CARE_V7_REPORTS_INVOICES_TRAVEL_CHECKLISTS";
...
function OwnerForm({ value, onChange }) { return <div style={S.formGrid}>{["name","phone","email","invoice_email","address","emergency_contact_name","emergency_contact_phone","default_mileage"].map(k=><Field key={k} label={k.replaceAll("_"," ")}><input type={k.includes("mileage")?"number":"text"} value={value[k]||""} onChange={e=>onChange({...value,[k]:e.target.value})}/></Field>)}<Field label="Access instructions"><textarea value={value.access_instructions||""} onChange={e=>onChange({...value,access_instructions:e.target.value})}/></Field><Field label="House instructions"><textarea value={value.house_instructions||""} onChange={e=>onChange({...value,house_instructions:e.target.value})}/></Field><Field label="Billing notes"><textarea value={value.billing_notes||""} onChange={e=>onChange({...value,billing_notes:e.target.value})}/></Field><Field label="Payment notes"><textarea value={value.payment_notes||""} onChange={e=>onChange({...value,payment_notes:e.target.value})}/></Field><Field label="Notes"><textarea value={value.notes||""} onChange={e=>onChange({...value,notes:e.target.value})}/></Field></div>; }
function PetForm({ value, onChange, vetClinics = [], onSaveVetClinic }) {
  ...
  return <div style={S.stack}>
    <div style={S.photoPickerBox}>
      ...
      <div style={S.row}>
        <label style={S.secondaryBtn}>Add photo from camera/gallery<input style={{display:"none"}} type="file" accept="image/*" capture="environment" onChange={e=>handlePhotoFile(e.target.files?.[0])}/></label>
        {value.photo_url && <button style={S.dangerMini} onClick={()=>onChange({...value,photo_url:""})}>Remove Photo</button>}
      </div>
    </div>
    <div style={S.formGrid}>
      {["name","species","breed","color_description","age_text","weight","sex","spayed_neutered","default_mileage"].map(k=><Field key={k} label={k.replaceAll("_"," ")}><input type={k.includes("mileage")?"number":"text"} value={value[k]||""} onChange={e=>onChange({...value,[k]:e.target.value})}/></Field>)}
      ...
      {["feeding_instructions","medication_instructions","medical_conditions","allergies","emergency_instructions","behavior_notes","leash_harness_notes","favorite_things","hide_spots","care_notes"].map(k=><Field key={k} label={k.replaceAll("_"," ")}><textarea value={value[k]||""} onChange={e=>onChange({...value,[k]:e.target.value})}/></Field>)}
    </div>
    <div style={S.detailBox}>
      <b>Quick add vet clinic</b>
      <small style={S.muted}>Add common clinics once, then choose them from the pet profile dropdown.</small>
      <div style={S.formGrid}>
      ...
      <button style={S.secondaryBtn} onClick={()=>{ if(newVet.clinic_name.trim()){ onSaveVetClinic(newVet); setNewVet({ clinic_name:"", phone:"", emergency_phone:"", address:"", notes:"" }); }}}>Save Vet Clinic</button>
    </div>
  </div>;
}
...
const S = {
  ...
  header:{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"18px 16px 8px",maxWidth:1180,margin:"0 auto"}, kicker:{fontSize:11,color:"#8b7d73",letterSpacing:1,textTransform:"uppercase"}, title:{fontSize:32,margin:0,fontWeight:800,color:"#211816"}, main:{maxWidth:1180,margin:"0 auto",padding:"8px 14px 30px"}, stack:{display:"grid",gap:14}, grid3:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:12}, twoCol:{display:"grid",gridTemplateColumns:"0.85fr 1.15fr",gap:14}, twoColBalanced:{display:"grid",gridTemplateColumns:"320px minmax(0,1fr)",gap:14}, subTabs:{display:"flex",gap:8,flexWrap:"wrap",margin:"8px 0 14px"}, subTab:{border:"1px solid #e1d2c5",background:"#fff",borderRadius:999,padding:"9px 12px",fontWeight:800}, subTabActive:{border:"1px solid #d9783f",background:"#fff2e8",borderRadius:999,padding:"9px 12px",fontWeight:900,color:"#763b15"}, detailBox:{border:"1px solid #eadfd6",borderRadius:20,padding:14,background:"#fffdfb",display:"grid",gap:12}, infoGrid:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))",gap:10}, visitHistory:{display:"grid",gridTemplateColumns:"1fr auto auto auto",gap:10,alignItems:"center",border:"1px solid #f0e4da",borderRadius:14,padding:10,background:"#fff"}, card:{background:"rgba(255,255,255,.92)",border:"1px solid #eadfd6",borderRadius:22,padding:16,boxShadow:"0 12px 30px rgba(70,50,35,.08)",textAlign:"left"}, metric:{background:"#fff",border:"1px solid #eadfd6",borderRadius:20,padding:16,textAlign:"left",boxShadow:"0 10px 22px rgba(70,50,35,.06)",display:"grid",gap:3}, muted:{color:"#6b7280"}, error:{maxWidth:1180,margin:"8px auto",padding:12,borderRadius:14,background:"#fff1f2",color:"#9f1239",textAlign:"left"}, toast:{position:"fixed",top:16,left:"50%",transform:"translateX(-50%)",zIndex:50,background:"#15251d",color:"#fff",padding:"10px 18px",borderRadius:999,animation:"successPop 1.4s ease"}, saving:{position:"fixed",right:16,bottom:95,background:"#111827",color:"#fff",padding:"10px 14px",borderRadius:14,zIndex:60}, bottomNav:{position:"fixed",bottom:0,left:0,right:0,display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,padding:"10px 12px calc(10px + env(safe-area-inset-bottom))",background:"rgba(255,255,255,.92)",borderTop:"1px solid #eadfd6",backdropFilter:"blur(12px)",zIndex:40}, navBtn:{border:"1px solid #e7d8ca",background:"#fff",borderRadius:18,padding:"14px 8px",fontWeight:800,color:"#6b5b50"}, navActive:{border:"1px solid #d99058",background:"#fff2e8",borderRadius:18,padding:"14px 8px",fontWeight:900,color:"#763b15",boxShadow:"0 8px 16px rgba(217,144,88,.2)"}, primaryBtn:{border:0,background:"#d9783f",color:"#fff",borderRadius:16,padding:"12px 16px",fontWeight:900}, secondaryBtn:{border:"1px solid #d9c7b8",background:"#fff",color:"#53392a",borderRadius:16,padding:"11px 14px",fontWeight:800}, ghostBtn:{border:"1px solid #e4d6c9",background:"rgba(255,255,255,.65)",borderRadius:14,padding:"10px 12px",fontWeight:800}, dangerBtn:{border:0,background:"#b91c1c",color:"#fff",borderRadius:16,padding:"11px 14px",fontWeight:800}, primaryMini:{border:0,background:"#d9783f",color:"#fff",borderRadius:12,padding:"8px 10px",fontWeight:800}, secondaryMini:{border:"1px solid #d9c7b8",background:"#fff",borderRadius:12,padding:"8px 10px",fontWeight:800}, dangerMini:{border:0,background:"#fee2e2",color:"#991b1b",borderRadius:12,padding:"8px 10px",fontWeight:800}, formGrid:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(240px,1fr))",gap:12}, field:{display:"grid",gap:5,fontWeight:800,color:"#544236"}, check:{display:"flex",gap:8,alignItems:"center",fontWeight:750}, row:{display:"flex",gap:10,alignItems:"center",flexWrap:"wrap"}, list:{display:"grid",gap:8,maxHeight:420,overflow:"auto"}, listBtn:{textAlign:"left",border:"1px solid #eadfd6",background:"#fff",borderRadius:15,padding:12,display:"grid",gap:3}, listActive:{textAlign:"left",border:"1px solid #d99058",background:"#fff2e8",borderRadius:15,padding:12,display:"grid",gap:3}, petCards:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(180px,1fr))",gap:12}, petCard:{border:"1px solid #eadfd6",background:"#fff",borderRadius:18,padding:12,display:"grid",gap:7}, petCardActive:{border:"2px solid #d9783f",background:"#fff7ef",borderRadius:18,padding:12,display:"grid",gap:7}, petPhoto:{width:62,height:62,borderRadius:18,objectFit:"cover"}, petPhotoBig:{width:"100%",maxHeight:260,borderRadius:22,objectFit:"cover"}, photoSmall:{width:62,height:62,borderRadius:18,background:"#f2e2d2",display:"grid",placeItems:"center",fontWeight:900,fontSize:26}, photoBlank:{height:140,borderRadius:20,background:"#f2e2d2",display:"grid",placeItems:"center",fontWeight:900}, petPickWrap:{display:"flex",gap:8,flexWrap:"wrap",margin:"10px 0"}, pickBtn:{border:"1px solid #e1d2c5",background:"#fff",borderRadius:999,padding:"10px 13px",fontWeight:800}, pickActive:{border:"1px solid #d9783f",background:"#fff2e8",color:"#763b15",borderRadius:999,padding:"10px 13px",fontWeight:900}, cards:{display:"grid",gridTemplateColumns:"repeat(auto-fill,minmax(220px,1fr))",gap:12,marginTop:12}, smallCard:{border:"1px solid #eadfd6",background:"#fff",borderRadius:17,padding:12,display:"grid",gap:5}, visitCard:{border:"1px solid #eadfd6",borderRadius:18,padding:13,display:"grid",gridTemplateColumns:"1fr auto",gap:12,alignItems:"center",marginTop:10}, cardActions:{display:"flex",gap:7,alignItems:"center",flexWrap:"wrap",justifyContent:"flex-end"}, petLine:{display:"flex",gap:6,flexWrap:"wrap",marginTop:6}, petChip:{border:0,background:"#e9f7ef",borderRadius:999,padding:"6px 9px",fontWeight:800,color:"#14532d"}, status:{fontSize:12,fontWeight:900,color:"#6b7280"}, officeNav:{display:"flex",gap:8,flexWrap:"wrap"}, officeBtn:{border:"1px solid #e1d2c5",background:"#fff",borderRadius:999,padding:"10px 13px",fontWeight:800}, officeActive:{border:"1px solid #d9783f",background:"#fff2e8",borderRadius:999,padding:"10px 13px",fontWeight:900}, reportRow:{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))",gap:10,alignItems:"center",borderBottom:"1px solid #f0e4da",padding:"10px 0"}, billingRow:{display:"grid",gridTemplateColumns:"auto minmax(0,1fr) auto auto",gap:10,alignItems:"center",border:"1px solid #f0e4da",borderRadius:14,padding:10,background:"#fff"}, billingRowCompact:{display:"grid",gridTemplateColumns:"minmax(0,1fr) auto auto auto",gap:12,alignItems:"center",border:"1px solid #f0e4da",borderRadius:14,padding:10,background:"#fff"}, checklistRow:{display:"grid",gridTemplateColumns:"minmax(0,1fr) auto",gap:10,alignItems:"center",border:"1px solid #f0e4da",borderRadius:12,padding:10,background:"#fff"}, subList:{gridColumn:"1/-1",display:"flex",gap:6,flexWrap:"wrap",color:"#6b7280"}, modalShade:{position:"fixed",inset:0,background:"rgba(30,20,10,.38)",zIndex:80,display:"grid",placeItems:"center",padding:14}, modal:{width:"min(900px,100%)",maxHeight:"88svh",overflow:"auto",background:"#fff",borderRadius:24,padding:16,boxShadow:"0 30px 80px rgba(0,0,0,.25)",textAlign:"left"}, modalHead:{display:"flex",justifyContent:"space-between",gap:10,alignItems:"center",borderBottom:"1px solid #eee",paddingBottom:10,marginBottom:12}, photoPickerBox:{border:"1px solid #eadfd6",borderRadius:20,padding:12,background:"#fff",display:"grid",gap:10}, petInfo:{display:"grid",gap:10}, info:{border:"1px solid #e5e7eb",borderRadius:14,padding:12,background:"#fff"}, infoDanger:{border:"1px solid #fecaca",borderRadius:14,padding:12,background:"#fff1f2"},
};
